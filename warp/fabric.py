...
from warp.dsl.types import quaternion, matrix, vector, noncontiguous_array_base, T, ARRAY_TYPE_FABRIC, array, \
    ARRAY_TYPE_FABRIC_INDEXED, check_index_array
...
class fabricarray(noncontiguous_array_base[T]):
    ...
    def vars(self):
        ...
        if fabricarray._vars is None:
            fabricarray._vars = {"size": Var("size", types.uint64)}
...
class indexedfabricarray(noncontiguous_array_base[T]):
    ...
    def vars(self):
        ...
        if indexedfabricarray._vars is None:
            indexedfabricarray._vars = {"size": Var("size", types.uint64)}
